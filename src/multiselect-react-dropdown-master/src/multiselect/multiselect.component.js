...
export class Multiselect extends React.Component {
    ...
    return Object.keys(groupedObject).map(obj => {
			return (
				<React.Fragment key={obj}>
					<li className={ms.groupHeading} style={groupedObject[obj][0].style ? groupedObject[obj][0].style :style['groupHeading']}>{obj}</li>
  ...
  renderSelectedList() {
    const { isObject = false, displayValue, displayKey, style, singleSelect } = this.props;
    const { selectedValues, closeIconType } = this.state;
    let newValues = JSON.parse(JSON.stringify(selectedValues))
    for (const value of newValues) {
      if(value.style){
        if(value.style.color){
          value.style.background = value.style.color
          delete value.style.color
        }
      }
